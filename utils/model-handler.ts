...
const copyModelToDocumentDirectory = async (): Promise<void> => {
  try {
    ...
    if (!modelInfo.exists) {
      console.log("Sao chép model TFLite...");
      
      try {
        // Cách 1: Thử với Asset.fromModule
        const modelAsset = Asset.fromModule(require("../assets/models/CNN.tflite"));
        await modelAsset.downloadAsync();
        
        const sourceUri = modelAsset.localUri || modelAsset.uri;
        await FileSystem.copyAsync({
          from: sourceUri,
          to: MODEL_PATH,
        });
        console.log("Model TFLite đã được sao chép thành công");
        
      } catch (error) {
        console.log("Cách 1 thất bại, thử cách 2...");
        
        // Cách 2: Sử dụng FileSystem.downloadAsync trực tiếp
        const modelAsset = Asset.fromModule(require("../assets/models/CNN.tflite"));
        await FileSystem.downloadAsync(modelAsset.uri, MODEL_PATH);
        console.log("Model TFLite đã được download thành công");
      }
    }

    // Xử lý file JSON - Không cần sao chép, chỉ cần tạo file trong document directory
    const labelsInfo = await FileSystem.getInfoAsync(LABELS_PATH);
    if (!labelsInfo.exists) {
      console.log("Tạo file labels.json...");
      
      // Load labels từ require (đã thành công như log cho thấy)
      const labelsData = require("../assets/models/labels.json");
      
      // Ghi file JSON vào document directory
      await FileSystem.writeAsStringAsync(
        LABELS_PATH, 
        JSON.stringify(labelsData, null, 2)
      );
      console.log("File labels.json đã được tạo thành công");
    }

    console.log("Hoàn thành copyModelToDocumentDirectory");
    
  } catch (error) {
    console.error("Lỗi chi tiết khi sao chép model hoặc labels:", error);
    let errorMessage = "";
    ...
    throw new Error(`Không thể sao chép model từ assets: ${errorMessage}`);
  }
};
